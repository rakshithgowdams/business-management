/*
  ...
  3. Functions
    - `encrypt_api_key(key text)` - encrypts an API key using a server-side secret
    - `decrypt_api_key(encrypted text)` - decrypts an API key (only callable via service role)

  4. Important Notes
    - The encryption uses pgcrypto with AES-256
    - The encryption passphrase is derived from the database's built-in secret
    - Only the edge function (service role) can decrypt keys
    - The client only ever sees the key_hint (last 4 chars)
*/
