...
import { Loader2, Download, Sparkles, Upload, ChevronDown, X, LayoutTemplate, Info, PlusCircle, CheckCircle2 } from 'lucide-react';
    ...
    <div className="relative rounded-2xl overflow-hidden border border-white/5 bg-dark-800 aspect-square">
      ...
      <div
        ...
        style={{
          ...
          animation: 'i2iShimmer 2s ease-in-out infinite',
        }}
      />
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 px-4">
        ...
        <div className="text-center">
          <p
            ...
            style={{ animation: 'i2iFloat 2s ease-in-out infinite' }}
          >
            Generating
          </p>
        ...
        <div className="flex gap-1 mt-1">
          {[0, 1, 2].map((i) => (
            <div
              ...
              style={{ animation: `i2iBounce 1s ease-in-out ${i * 0.2}s infinite` }}
            />
          ))}
        </div>
      </div>
...
function GeneratedImageCard({ url, label, onDownload }: { url: string; label: string; onDownload: () => void }) {
  return (
    <div className="group relative rounded-2xl overflow-hidden border border-white/5 aspect-square" style={{ animation: 'i2iFadeIn 0.5s ease-out' }}>
      <img src={url} alt="Transformed" className="w-full h-full object-cover" />
      ...
      <div className="absolute top-2 left-2 w-6 h-6 rounded-full bg-teal-500/80 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all">
...
export default function ImageToImage({ onSlotsChange }: ImageToImageProps) {
  const { user } = useAuth();
  const rawCustomModels = useCustomModels(user?.id, ['image-to-image', 'image-to-image']);
  ...
  const customModels = rawCustomModels.filter(
    (cm) => cm.category === 'image-to-image'
  );

  const allModels: I2IModel[] = customModels.map((cm) => ({
    id: cm.id,
    label: cm.name,
  ...
  const uploadSourceImage = async (file: File) => {
    ...
    try {
      const assetId = `i2i-source-${crypto.randomUUID()}`;
      const mimeType = file.type || 'image/jpeg';
      const result = await uploadMediaToStorage(user.id, assetId, file, mimeType);
      if (!result) {
  ...
  const buildBody = (sourceUrl: string): Record<string, unknown> => {
    ...
    if (input.strength !== undefined) input.strength = strength;
  ...
  const generateSingleImage = async (index: number, apiKey: string, sourceUrl: string): Promise<string | null> => {
    ...
    try {
      ...
      return await pollSingleTask(index, taskId, apiKey, pollUrl, parser);
    } catch (err) {
      stopElapsedTimer(index);
      setSlots((prev) => prev.map((s) => s.index === index ? { ...s, status: 'failed' } : s));
  ...
  const handleGenerate = async () => {
    ...
    if (!prompt.trim()) { toast.error('Enter a prompt describing the transformation'); return; }
    ...
    try {
      const promises = Array.from({ length: count }, (_, i) =>
        generateSingleImage(i, apiKey, uploadedSourceUrl)
      );
      const results = await Promise.all(promises);
      const successUrls = results.filter((u): u is string => u !== null);

      if (successUrls.length > 0) {
        toast.success(`${successUrls.length} of ${count} image${count > 1 ? 's' : ''} transformed`);
        await saveToGallery(successUrls);
      } else {
        toast.error('All transformations failed');
      }
    } catch (err) {
  ...
  const handleDownload = (url: string) => {
    ...
    a.href = url;
    a.download = `ai-i2i-${Date.now()}.png`;
    a.target = '_blank';
  ...
  if (allModels.length === 0) {
    return (
      <div className="glass-card rounded-xl p-12 flex flex-col items-center justify-center text-center">
        ...
        <h3 className="text-lg font-semibold text-white mb-2">No Image-to-Image Models Added</h3>
        <p className="text-sm text-gray-400 max-w-sm mb-6">
          Add your custom image-to-image models via the API Console to start transforming images.
          Go to <span className="text-teal-400">Settings &rarr; API Console</span> and add a model with type "Image to Image".
        </p>
        <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-teal-500/10 border border-teal-500/20 text-teal-400 text-sm font-medium">
    ...
    <div className="space-y-5">
      ...
      <div className="glass-card rounded-xl p-6">
        <div className="space-y-4">
              ...
              {showModelPicker && (
                <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-dark-800 border border-white/10 rounded-xl shadow-2xl max-h-80 overflow-y-auto">
                  {groupedCategories.map((cat) => (
                    <div key={cat}>
                      <div className="px-4 py-2 text-[10px] font-bold uppercase tracking-widest border-b border-white/5 sticky top-0 text-teal-400 bg-teal-500/5">{cat}</div>
                      {allModels.filter((m) => m.category === cat).map((m) => (
                        <button
                          key={m.id}
                          onClick={() => { setSelectedModel(m.id); setShowModelPicker(false); }}
                          className={`w-full text-left px-4 py-2.5 text-sm hover:bg-white/5 transition-colors flex items-center justify-between ${m.id === selectedModel ? 'text-teal-400 bg-teal-500/5' : 'text-gray-300'}`}
                        >
                          <div className="flex items-center gap-2">
                            <span>{m.label}</span>
                            <span className="px-1.5 py-0.5 rounded text-[8px] font-bold bg-emerald-500/10 text-emerald-400">CUSTOM</span>
                          </div>
                          {m.id === selectedModel && <span className="text-[10px] font-bold text-teal-400">SELECTED</span>}
                        </button>
                      ))}
                    </div>
                  ))}
          ...
          {capabilities && (
            <div className="flex items-start gap-2 px-3 py-2 rounded-xl bg-teal-500/5 border border-teal-500/10">
              <Info className="w-3.5 h-3.5 text-teal-400 flex-shrink-0 mt-0.5" />
              <div>
                <p className="text-[11px] text-teal-400 font-medium">Model Capabilities</p>
                <div className="flex flex-wrap gap-1.5 mt-1">
                  ...
                  {capabilities.hasAspectRatio && <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-teal-500/10 text-teal-400">Aspect Ratio</span>}
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-2">
              Source Image
              {isUploadingSource && (
                <span className="ml-2 text-[10px] text-teal-400 inline-flex items-center gap-1">
            ...
            {sourcePreview ? (
                  ...
                  {isUploadingSource && (
                    <div className="absolute inset-0 rounded-xl bg-black/50 flex items-center justify-center">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-xs text-teal-300 font-medium">Uploading to cloud...</p>
                      </div>
                    </div>
                  )}
                  {uploadedSourceUrl && !isUploadingSource && (
              ...
              <button
                ...
                className="flex items-center justify-center w-full max-w-xs h-48 rounded-xl border-2 border-dashed border-white/10 hover:border-teal-400/30 transition-colors group"
              >
                <div className="text-center">
                  <Upload className="w-8 h-8 text-gray-500 group-hover:text-teal-400 mx-auto mb-2 transition-colors" />
                  <p className="text-sm text-gray-500 group-hover:text-gray-300 transition-colors">Upload Source Image</p>
          ...
          {appliedTemplate?.reference_image_url && (
            <div className="flex items-start gap-3 p-3 rounded-xl bg-teal-500/5 border border-teal-500/15">
              ...
              <div>
                <p className="text-xs font-medium text-teal-400">Template Reference Image</p>
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">
              ...
              {appliedTemplate && <span className="ml-2 text-[10px] text-teal-400">from template: {appliedTemplate.name}</span>}
            </label>
            <textarea
              value={prompt}
              ...
              className="w-full px-4 py-3 bg-dark-800 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-teal-400 resize-none"
            />
          </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {ASPECT_RATIOS.map((ar) => (
                    <button key={ar.value} onClick={() => setAspectRatio(ar.value)} className={`px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all ${aspectRatio === ar.value ? 'bg-teal-500 text-white' : 'bg-dark-800 border border-white/10 text-gray-400 hover:text-white'}`}>
                      {ar.label}
                    </button>
                  ))}
                </div>
                  ...
                  {RESOLUTIONS.map((r) => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative px-2.5 py-1.5 rounded-lg text-xs font-bold transition-all
                          ${active ? 'bg-teal-500 text-white shadow-sm shadow-teal-500/30' : ''}
                          ${supported && !active ? 'bg-dark-800 border border-white/10 text-gray-400 hover:text-white hover:border-teal-500/30 cursor-pointer' : ''}
                  ...
                  {[1, 2, 3, 4].map((n) => (
                    <button
                      ...
                      className={`relative px-3 py-1.5 rounded-lg text-xs font-medium transition-all disabled:opacity-40
                        ${count === n ? 'bg-teal-500 text-white' : 'bg-dark-800 border border-white/10 text-gray-400 hover:text-white hover:border-teal-500/30'}
                      `}
                    >
                      {n}
          ...
          <button
            ...
            disabled={isAnyGenerating || !sourceImage || !uploadedSourceUrl || isUploadingSource || !prompt.trim() || !selectedModel}
            className="w-full py-3.5 rounded-xl bg-gradient-to-r from-teal-500 to-cyan-500 text-white font-semibold flex items-center justify-center gap-2 disabled:opacity-50 hover:from-teal-600 hover:to-cyan-600 transition-all hover:shadow-lg hover:shadow-teal-500/20 active:scale-[0.99]"
          >
            {isAnyGenerating ? (
              <>
                <Loader2 className="w-5 h-5 animate-spin" />
                <span>Transforming {completedCount}/{count} images...</span>
                {generatingCount > 0 && (
              ...
              <>
                ...
                Transform {count > 1 ? `${count} Images` : 'Image'} with {selectedModelDef?.label || 'Model'}
                {capabilities?.hasResolution && (
